...
pub fn serialize_opt_enum_or_unknown<E: EnumFull, S: serde::Serializer>(
  e: &Option<EnumOrUnknown<E>>,
  s: S,
) -> Result<S::Ok, S::Error> {
  if let Some(e) = e {
    serialize_enum_or_unknown(e, s)
  } else {
    s.serialize_unit()
  }
}

pub fn deserialize_opt_enum_or_unknown<'de, E: EnumFull, D: serde::Deserializer<'de>>(
  d: D,
...
pub fn deserialize_enum_or_unknown<'de, E: EnumFull, D: serde::Deserializer<'de>>(
  ...
  impl<'de, E: EnumFull> serde::de::Visitor<'de> for DeserializeEnumVisitor<E> {
    ...
    where
      R: serde::de::Error,
    {
      Err(serde::de::Error::custom("missing enum value".to_string()))
    }
  }
